...
export default function AdminUsuarios() {
  ...
  const [form, setForm] = useState({ nome: '', email: '', cliente_id: '', status: 'ativo' });
  ...
  const fetchData = async () => {
    ...
    setUsuarios((data || []).map((u: any) => ({
      id: u.id, email: u.email, nome: u.nome, status: u.status, created_at: u.created_at,
      cliente_id: u.cliente_id, cliente_nome: u.clientes?.nome || '-', role: rolesMap.get(u.id) || 'user',
    })));
    setLoading(false);
  };
  ...
  const resetForm = () => { setForm({ nome: '', email: '', cliente_id: '', status: 'ativo' }); setEditing(null); setOpenModal(false); };
  ...
  const columns = [
    ...
    { key: 'cliente_nome', header: 'Cliente' },
    {
      ...
      render: (v: any) => (
        <span style={{ padding: '4px 12px', borderRadius: 999, fontSize: 12, fontWeight: 600, background: v === 'admin' ? 'rgba(6,182,212,0.15)' : 'rgba(148,163,184,0.15)', color: v === 'admin' ? '#06b6d4' : '#94a3b8' }}>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => setOpenModal(true)} style={heroButton}><PlusIcon /> Novo Usuário</button>
      </div>

      <div style={{ position: 'relative', maxWidth: 420 }}>
        <span style={{ position: 'absolute', left: 14, top: '50%', transform: 'translateY(-50%)', color: '#94a3b8', display: 'flex' }}><SearchIcon /></span>
        <input placeholder="Buscar usuários..." value={search} onChange={e => setSearch(e.target.value)} style={searchInput} />
      </div>
        ...
        <div style={overlay} onClick={() => resetForm()}>
          <div style={modalBox} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <h3 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>{editing ? 'Editar Usuário' : 'Novo Usuário'}</h3>
                <p style={{ fontSize: 13, color: '#94a3b8', margin: '4px 0 0' }}>{editing ? 'Edite as informações do usuário.' : 'Cadastre um novo usuário no sistema.'}</p>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, marginTop: 8 }}>
              ...
              <button onClick={handleSave} disabled={saving} style={heroButton}>{saving ? 'Salvando...' : editing ? 'Salvar' : 'Cadastrar Usuário'}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {deleteTarget && (
        <div style={overlay} onClick={() => setDeleteTarget(null)}>
          <div style={modalBox} onClick={e => e.stopPropagation()}>
            ...
            <p style={{ color: '#94a3b8', margin: '0 0 20px', fontSize: 14 }}>Essa ação não pode ser desfeita. O usuário "{deleteTarget.nome}" será removido.</p>
